...
async def pugach_menu(message: Message, config: FromDishka[AppConfig]):
...
async def pugach_dota(callback: CallbackQuery, config: FromDishka[AppConfig], heroes: FromDishka[HeroesData]):
	text = await get_dota_stats_text(steam_id32=config.steam.pugach_steam_id32, heroes=heroes)
